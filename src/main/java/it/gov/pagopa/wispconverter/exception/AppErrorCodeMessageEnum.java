...
public enum AppErrorCodeMessageEnum {
    ...
    PARSING_INVALID_XML_NODES(1004, "XML parsing error", "Error while parsing payload. The list of nodes extracted from document must be greater than zero, but they are zero.", HttpStatus.BAD_REQUEST),
    PARSING_INVALID_ZIPPED_PAYLOAD(1005, "Parse error", "Error while parsing payload. Cannot unzip payload correctly.", HttpStatus.BAD_REQUEST),
