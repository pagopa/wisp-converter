...
public class ResponseValidator {
     ...
     */
    @AfterReturning(pointcut = "execution(* it.gov.pagopa.wispconverter.controller.*.*(..))", returning = "result")
    ...
    private <T> void validateResponse(ResponseEntity<T> response) {
        if (response.getBody() != null) {
            ...
            if (!validationResults.isEmpty()) {
                ...
                throw new AppException(AppErrorCodeMessageEnum.ERROR, "Invalid response");
            }
        }
    }
}
